...
**Status**: M13 complete. M13.1 unified sales reporting shipped (backend `6e47139` + frontend `e2eef27`): `GET /tenants/me/analytics/sales`, KPI dashboard, by-channel + by-payment-method tables, 7d/30d/90d presets, revenue = non-cancelled, cancelled reported separately. M13.2 revenue analytics shipped (backend `4a34b76` + frontend `d03a518`): `GET /tenants/me/analytics/revenue` with by-day (incl. shipping) and top-10-products (excl. shipping) breakdowns, Revenue Over Time + Top Products tables on the dashboard reports page. M13.3 POS today widgets shipped (backend `cff1004` + frontend `8da7809`): `GET /tenants/me/analytics/pos-today` single-day POS snapshot, "POS — Today" panel on the reports page with KPIs, top-5 products, and payment methods; open-shift summary deferred. M13.4 repeat customer analytics shipped (backend `94a0072` + frontend `aaedb3b`): `GET /tenants/me/analytics/repeat-customers` new-vs-returning metrics (orders.customer_id identity, B1 first-ever-order partition, anonymous orders reported separately), "Customers — New vs Returning" block on the reports page with New/Returning/Repeat Rate/Anonymous Orders KPIs + Top Returning Customers table.
...
| M12 — Operations & Variants | Both tracks | Complete — M12.1 product variants backend shipped (schema/CRUD/order + inventory wiring, backend only); M12.2 variant UI (dashboard/POS/storefront + Arabic i18n) + M12.2.1 POS visibility fix shipped; M12.3 POS shift management shipped (backend + frontend); M12.4 POS cancel reason shipped (backend + frontend); M12.5a delivery address foundation shipped (backend + frontend); M12.5b shipping methods/fees shipped (backend + frontend); M12.6 order fulfillment status chain shipped (backend + frontend); M12.7 customer order history shipped (backend + frontend) |
| M13 — Omnichannel Reporting & Polish | Both tracks (convergence) | Complete — M13.1 unified sales reporting shipped (backend `6e47139` + frontend `e2eef27`); M13.2 revenue analytics shipped (backend `4a34b76` + frontend `d03a518`); M13.3 POS today widgets shipped (backend `cff1004` + frontend `8da7809`); M13.4 repeat customer analytics shipped (backend `94a0072` + frontend `aaedb3b`) |
