...
| 8.9 | Set up ALB + CloudFront + WAF | Claude | In progress | P1: ALB `saas-alb` created (internet-facing, HTTP :80), target group attached to `saas-backend`, health checks passing, public DNS working. Remaining: HTTPS/ACM for `api.<domain>` (P2), CloudFront + WAF (P3), CORS/domain wiring (P4). Blocked by: production domain choice + DNS provider access (shared prerequisite with 8.10). |
| 8.10 | Set up Vercel project with wildcard subdomain | Claude | Not started | Frontend at `app.<domain>`, tenant subdomains at `*.app.<domain>`. Blocked by: production domain choice + DNS provider access (shared prerequisite with 8.9). |
