...
| 8.10 | Set up Vercel project with wildcard subdomain | Claude | In progress | Demo P1: Frontend deployed to Vercel at `https://ai-storefront-platform.vercel.app` (default domain, no custom domain yet). Backend `saas-backend:3` updated with `ALLOWED_ORIGINS=https://ai-storefront-platform.vercel.app`, CORS preflight verified. Dev auth enabled for demo. Caveats: cross-site refresh cookie not suitable yet, no seed data, dev auth mode. Remaining: seed demo data (Demo P2), custom domain `app.ramitestapp.top` (Demo P3, optional). |
