...
| 8.10 | Set up Vercel project with wildcard subdomain | Claude | In progress | Demo P1: Frontend deployed to Vercel at `https://ai-storefront-platform.vercel.app`. Backend CORS configured (`saas-backend:3`). Demo P2a: `COGNITO_MOCK=true` enabled on ECS (`saas-backend:4`), dev token login works, dashboard health widget verified. Frontend fix: dev-login token whitespace stripping (`.replace(/\s+/g,"")`) committed. Demo P2b: Tenant `rami-demo-store` created (PHP currency), 2 categories (Electronics, Accessories), 3 products (Wireless Headphones, Phone Case, USB-C Cable), storefront config set. Active tenant membership confirmed. Remaining: demo product images (approved packet), custom domain `app.ramitestapp.top` (Demo P3, optional). |
