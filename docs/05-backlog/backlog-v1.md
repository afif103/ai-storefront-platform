...
| 8.9 | Set up ALB + CloudFront + WAF | Claude | In progress | P1: ALB `saas-alb` created (internet-facing, HTTP :80→redirect 301→HTTPS). P2: ACM cert issued for `api.ramitestapp.top`, HTTPS :443 listener live with TLS 1.3 policy, https://api.ramitestapp.top/api/v1/health returns ok. Remaining: CloudFront + WAF (P3), CORS/domain wiring (P4). Domain: `ramitestapp.top` on Namecheap BasicDNS. |
| 8.10 | Set up Vercel project with wildcard subdomain | Claude | Not started | Frontend at `app.ramitestapp.top`, tenant subdomains at `*.app.ramitestapp.top`. |
