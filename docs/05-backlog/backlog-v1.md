...
| # | Task | Primary Implementor | Status | DoD |
|---|------|-------|--------|-----|
| 12.7a | Customer order history page | Claude | **Shipped** | Dashboard merchant-facing per-customer order history across online + POS sales (backend `2da9c5d` + frontend `3093fc9`). Backend `2da9c5d`: new `GET /tenants/me/customers/{customer_id}/orders` endpoint, member-gated, verifies the customer belongs to the tenant and returns `404` on missing/cross-tenant, returns that customer's orders filtered by `tenant_id` + `customer_id`, newest-first, with limit/offset, and reuses `OrderListItem` with no model/schema/migration changes; source-agnostic, so any storefront or POS order linked to the customer is included; 10 backend tests. Frontend `3093fc9`: new `/dashboard/customers` page with customer list, cursor Load more, per-customer order history table (Number / Date / Source / Status / Fulfillment / Total), Customers nav link, and 21 i18n keys (Arabic uses English fallback). **Note:** POS orders are contactless by API design (`customer_id` is `NULL`), so a POS sale appears here only if separately linked to a customer. **Deferred:** public customer-facing lookup → future OTP/magic-link packet; donations/pledges history; POS contact capture. |
...
| # | Task | Primary Implementor | Status | DoD |
|---|------|-------|--------|-----|
| 13.1a | Unified sales reporting: online orders + POS sales in combined views | Claude | **Shipped** | `GET /tenants/me/analytics/sales` (member-gated; `from`/`to` required; max 180d range; revenue = non-cancelled; cancelled reported separately as `cancelled_orders`/`cancelled_amount`; AOV 3dp, 0.000 when no non-cancelled orders); KPI cards (Total Sales / Total Orders / AOV / Storefront Sales / POS Sales / Cancelled); by-channel + by-payment-method tables; `payment_method` NULL renders as Unspecified; 7d/30d/90d range presets (default 30d); `dateRange` sends `to=today` (backend owns exclusive upper bound). Backend `6e47139`, frontend `e2eef27`. M13.2–M13.4 not started. |

### M13.2 — Revenue Analytics

| # | Task | Primary Implementor | Status | DoD |
|---|------|-------|--------|-----|
| 13.2a | Revenue analytics by channel, product, time period | Claude | **Shipped** | `GET /tenants/me/analytics/revenue` (member-gated; `from`/`to` required; max 180d range; revenue = non-cancelled). `by_day`: date, order_count, gross_sales (incl. shipping, from `total_amount`), storefront_sales, pos_sales. `top_products`: top 10 by gross_sales (excl. shipping, from line-item subtotal), with product_id, name, qty_sold. Dashboard reports page adds Revenue Over Time + Top Products tables on the same 7d/30d/90d presets, independent fetch/state from the M13.1 sales summary. Backend `4a34b76`, frontend `d03a518`. M13.3–M13.4 not started. |

### M13.3 — POS Dashboard Widgets

| # | Task | Primary Implementor | Status | DoD |
|---|------|-------|--------|-----|
| 13.3a | POS dashboard widgets: today's sales, top products | Claude | **Shipped** | `GET /tenants/me/analytics/pos-today?date=YYYY-MM-DD` (member-gated; required date; POS-only; non-cancelled on all queries incl. currency, fallback KWD; day window `[date, date+1d)`). Returns pos_sales, pos_order_count, by_payment_method (NULL preserved), top 5 products by line-subtotal revenue. "POS — Today" panel on `/dashboard/reports` (placement decision: reports page, not dashboard home): POS Sales/Orders Today KPIs + Top POS Products + POS Payment Methods tables, browser-local date via `localTodayString()`, independent fetch/state, empty state when no POS sales. Backend `cff1004`, frontend `8da7809`. Open-shift summary deferred (exists via `/pos/shifts/current`). M13.4 not started. |

### M13.4 — Customer Repeat-Purchase Tracking

| # | Task | Primary Implementor | Status | DoD |
|---|------|-------|--------|-----|
| 13.4a | Customer repeat-purchase tracking | Claude | **Shipped** | `GET /tenants/me/analytics/repeat-customers?from=YYYY-MM-DD&to=YYYY-MM-DD` (member-gated; from/to required; max 180d; non-cancelled only). Identity = `orders.customer_id`; B1 first-ever-order partition: new = first-ever non-cancelled order inside the window, returning = first-ever order predates the window; `customer_id IS NULL` orders counted only as anonymous_orders; repeat_rate 4dp (0.0 when empty); top_returning LIMIT 5 with safe display-name fallback. "Customers — New vs Returning" block on `/dashboard/reports` (after Revenue, before POS Today; preset-driven): New Customers / Returning Customers / Repeat Rate / Anonymous Orders KPIs + Top Returning Customers table (Customer/Orders/Lifetime Orders/Spent/First Order), empty state when no identified customers. Backend `94a0072`, frontend `aaedb3b`. **M13 complete.** |
...
| M12 — Operations & Variants | 7 packets (M12.1–M12.7) | Complete — M12.1 product variants backend shipped (schema/RLS/CRUD, cross-table SKU/barcode uniqueness, order + POS variant price/stock, variant-aware cancel/restore); M12.2 variant UI + M12.2.1 POS visibility fix shipped; M12.3 POS shift management shipped (backend `cb6a3c0` + frontend `0f3c057`: `pos_shifts` table/RLS + one-open-shift-per-tenant index, `orders.shift_id`, cashier-gated current/open/close endpoints, hard open-shift gate on POS sales, cash-only reconciliation summary, and the POS shift banner/open-close forms/checkout gate); M12.4 POS cancel reason shipped (backend `78654d8` + frontend `8d6d42a`: optional `{ reason }` on the full-order cancel, `orders.cancel_reason`, audit metadata, receipt/history display — full-order cancel only, not line-item void); M12.5a storefront delivery address foundation shipped (backend `90cfda1` + frontend `d2d4fc4`: `orders.shipping_address`, optional request field + `OrderDetailResponse`, checkout address/notes fields, dashboard display); M12.5b shipping methods/fees shipped (backend `190a72c` + frontend `e0bcfbf`: `storefront_config.shipping` JSONB methods, server-resolved fee on `orders.shipping_fee` / `orders.shipping_method`, dashboard editor + checkout selector); M12.6 order fulfillment status chain shipped (backend `b7ff3c8` + frontend `1defc24`: orthogonal `orders.fulfillment_status` `NULL → packed → shipped → delivered`, admin fulfillment endpoint, list badge + detail next-step control); M12.7 customer order history shipped (backend `2da9c5d` + frontend `3093fc9`: member-gated `GET /tenants/me/customers/{id}/orders`, dashboard customers list → per-customer order history across storefront + POS) |
| M13 — Omnichannel Reporting & Polish | 4 packets (M13.1–M13.4) | Complete — M13.1 unified sales reporting shipped (backend `6e47139` + frontend `e2eef27`); M13.2 revenue analytics shipped (backend `4a34b76` + frontend `d03a518`); M13.3 POS today widgets shipped (backend `cff1004` + frontend `8da7809`); M13.4 repeat customer analytics shipped (backend `94a0072` + frontend `aaedb3b`) |
