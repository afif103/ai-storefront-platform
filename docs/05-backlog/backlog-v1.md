...
| 8.9 | Set up ALB + CloudFront + WAF | Claude | In progress | P1: ALB `saas-alb` created (internet-facing, HTTP :80), target group attached to `saas-backend`, health checks passing, public DNS working. Remaining: HTTPS/ACM (P2), CloudFront + WAF (P3), CORS/domain wiring (P4). |
