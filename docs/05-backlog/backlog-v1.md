...
| 8.6 | Provision ElastiCache Redis (encryption in transit) | Claude | Not started | MVP temporary profile: Single-AZ `cache.t4g.micro`, in-transit + at-rest encryption, priv-data subnets. Planned upgrade to Multi-AZ when budget/usage justifies. ECS-only access via SG. No AUTH token for MVP. |
